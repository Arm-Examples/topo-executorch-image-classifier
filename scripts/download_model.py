...
def download(repo: str, filename: str, required: bool = True) -> str | None:
    try:
...
def main() -> None:
    ...
    download(repo, model)
    download(repo, "config.yaml")
    download(repo, "imagenet_classes.json", required=False)
    download(repo, "sample_input.jpg", required=False)
